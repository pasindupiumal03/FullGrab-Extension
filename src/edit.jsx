...
const EditPage = () => {
    ...
    useEffect(() => {
        ...
        chrome.storage.local.get(['capturedImage', 'originalCaptures', 'pageTitle', 'pageUrl', 'screenshotCounter'], (result) => {
    ...
    }, [textOverlays, shapes, drawingPaths, backgroundEnabled, selectedGradient, selectedColor, frameStyle]);
    ...
    const captureEditedCanvas = () => {
        return new Promise((resolve) => {
            ...
            import('html2canvas').then(html2canvas => {
                html2canvas.default(editCanvas, {
            ...
            if (result.success) {
                ...
                switch(platform) {
                    ...
                    case 'Instagram':
                        ...
                        alert('Instagram sharing: Please copy the image URL and share it manually in the Instagram app.\n\nImage URL: ' + imageUrl);
    ...
    const updateSelectedTextSize = (fontSize) => {
        if (!selectedTextId) return;
        setTextOverlays(textOverlays.map(t => 
            t.id === selectedTextId ? { ...t, fontSize: Number(fontSize) } : t
        ));
    };
    ...
    const handleShapeMouseMove = (e) => {
        ...
        if (!canvas) return;
        const rect = canvas.getBoundingClientRect();
        const newX = Math.max(0, Math.min(100, (e.clientX - rect.left) / rect.width * 100 - shapeDragOffset.x));
        const newY = Math.max(0, Math.min(100, (e.clientY - rect.top) / rect.height * 100 - shapeDragOffset.y));
    ...
    const updateSelectedShapeStyle = (style) => {
        if (!selectedShapeId) return;
        setShapes(shapes.map(s => 
            s.id === selectedShapeId ? { ...s, style } : s
        ));
    };
    ...
    const updateSelectedShapeRotation = (rotation) => {
        if (!selectedShapeId) return;
        setShapes(shapes.map(s => 
            s.id === selectedShapeId ? { ...s, rotation: Number(rotation) } : s
        ));
    };
    ...
    const getToolIcon = (toolId, isActive) => {
        ...
        const icons = {
            background: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>,
            frame: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><rect x="7" y="7" width="10" height="10" rx="1"/></svg>,
            resize: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 3 21 3 21 9"/><polyline points="9 21 3 21 3 15"/><line x1="21" y1="3" x2="14" y2="10"/><line x1="3" y1="21" x2="10" y2="14"/></svg>,
            crop: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 2v14a2 2 0 0 0 2 2h14"/><path d="M18 22V8a2 2 0 0 0-2-2H2"/></svg>,
            text: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="4 7 4 4 20 4 20 7"/><line x1="9" y1="20" x2="15" y2="20"/><line x1="12" y1="4" x2="12" y2="20"/></svg>,
            shapes: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>,
            brush: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="m9.06 11.9 8.07-8.06a2.85 2.85 0 1 1 4.03 4.03l-8.06 8.08"/><path d="M7.07 14.94c-1.66 0-3 1.35-3 3.02 0 1.33-2.5 1.52-2 2.02 1.08 1.1 2.49 2.02 4 2.02 2.2 0 4-1.8 4-4.04a3.01 3.01 0 0 0-3-3.02z"/></svg>,
            postout: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
        };
        return icons[toolId];
    };
        ...
        <>
            ...
            <header className="edit-header" style={{
                ...
                overflow: 'visible'
                    ...
                    {/* Download Buttons */}
                    <div className="edit-actions" style={{ display: 'flex', gap: '12px', alignItems: 'center', position: 'relative', overflow: 'visible' }}>
                    <div className="header-tooltip-wrapper">
            ...
            <div style={{ display: 'flex', flex: 1, overflow: 'hidden', height: 0 }}>
                ...
                <aside className="edit-sidebar" style={{
                    ...
                    {/* Tools */}
                    <div style={{ padding: '16px', minHeight: 'min-content' }}>
                        <div className="edit-tool-grid" style={{
                            display: 'grid',
                            ...
                            {tools.map(tool => (
                                <button
                                    ...
                                    style={{
                                        aspectRatio: '1',
                                        background: activeTool === tool.id ? 'linear-gradient(135deg, #a855f7 0%, #ec4899 100%)' : 'white',
                                        color: activeTool === tool.id ? 'white' : '#6b7280',
                                        border: activeTool === tool.id ? 'none' : '1px solid #e5e7eb',
                                        borderRadius: '12px',
                                        cursor: 'pointer',
                                        display: 'flex',
                                        flexDirection: 'column',
                                        ...
                                        minWidth: 0,
                                        overflow: 'visible',
                                        boxShadow: activeTool === tool.id ? '0 4px 12px rgba(168, 85, 247, 0.3)' : '0 1px 3px rgba(0,0,0,0.05)',
                                        transform: activeTool === tool.id ? 'translateY(-2px)' : 'translateY(0)'
                                    }}
                                >
                                    <div style={{ marginBottom: '2px' }}>{getToolIcon(tool.id, activeTool === tool.id)}</div>
                                    <span style={{ 
                                        fontSize: '9px', 
                                        fontWeight: 600,
                                        textAlign: 'center',
                                ...
                                <div style={{
                                    ...
                                    <div style={{
                                        ...
                                        <div style={{
                                            ...
                                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={backgroundEnabled ? '#a855f7' : '#6b7280'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                                                <path d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/>
                                            </svg>
                                            <span style={{ fontSize: '13px', fontWeight: 600, color: '#111827' }}>Add Background</span>
                                        </div>
                                        <button
                                            ...
                                            {solidColors.map((color, index) => (
                                                <button
                                                    ...
                                                    style={{
                                                        aspectRatio: '1',
                                                        background: color,
                                                        border: selectedColor === color ? '2px solid #a855f7' : '1px solid #e5e7eb',
                                                        borderRadius: '8px',
                                                        cursor: 'pointer'
                                                    }}
                                                />
                            ...
                            <div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                    {['shadow', 'border', 'glow'].map(style => {
                                        const getFrameIcon = (frameStyle, isActive) => {
                                            const color = isActive ? 'white' : '#6b7280';
                                            const icons = {
                                                shadow: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/></svg>,
                                                border: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/></svg>,
                                                glow: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
                                            };
                                            return icons[frameStyle];
                                        };
                                        ...
                                        return (
                                            <button
                                                ...
                                                style={{
                                                    padding: '12px 14px',
                                                    background: frameStyle === style ? 'linear-gradient(135deg, #a855f7 0%, #ec4899 100%)' : 'white',
                                                    color: frameStyle === style ? 'white' : '#6b7280',
                                                    border: frameStyle === style ? 'none' : '1px solid #e5e7eb',
                                                    borderRadius: '10px',
                                                    cursor: 'pointer',
                                                    fontWeight: 600,
                                                    fontSize: '13px',
                                                    ...
                                                    gap: '10px',
                                                    transition: 'all 0.2s',
                                                    boxShadow: frameStyle === style ? '0 4px 12px rgba(168, 85, 247, 0.3)' : '0 1px 3px rgba(0,0,0,0.05)'
                                                }}
                                            >
                                                {getFrameIcon(style, frameStyle === style)}
                                                <span>{style === 'shadow' ? 'Drop Shadow' : style === 'border' ? 'Border' : 'Glow Effect'}</span>
                                            </button>
                                        );
                                    })}
                                </div>
                        ...
                        {activeTool === 'text' && (
                            <div>
                                <div style={{
                                    ...
                                    <div style={{ fontSize: '12px', fontWeight: 600, color: '#6b7280', marginBottom: '8px' }}>Add Text</div>
                                    <input
                                        type="text"
                                ...
                                {textOverlays.length > 0 && (
                                    <div>
                                        <div style={{ fontSize: '12px', fontWeight: 600, color: '#6b7280', marginBottom: '8px' }}>Text Overlays ({textOverlays.length})</div>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                            {textOverlays.map(overlay => (
                                                <div key={overlay.id} style={{
                                                    ...
                                                    <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{overlay.text}</span>
                                                    <button onClick={() => handleRemoveText(overlay.id)} style={{
                                                        background: '#ef4444',
                                                        color: 'white',
                                                        border: 'none',
                                                        borderRadius: '4px',
                                ...
                                {selectedTextId && textOverlays.find(t => t.id === selectedTextId) && (
                                    <div style={{
                                        ...
                                        <div style={{ fontSize: '12px', fontWeight: 600, color: '#a855f7', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            <span>✏️</span> Edit Selected Text
                                        </div>
                                        <div style={{ marginBottom: '12px' }}>
                                            <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Font Size: {textOverlays.find(t => t.id === selectedTextId)?.fontSize}px</div>
                                            <input
                                                type="range"
                                                min="12"
                                                max="72"
                                        ...
                                        <div>
                                            <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Text Color</div>
                                            <input
                                                type="color"
                                        ...
                                        <div style={{ fontSize: '10px', color: '#9ca3af', marginTop: '8px', textAlign: 'center' }}>💡 Drag text on canvas to reposition</div>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {activeTool === 'shapes' && (
                            <div>
                                <div style={{ fontSize: '12px', fontWeight: 600, color: '#6b7280', marginBottom: '8px' }}>Add Shapes</div>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '8px', marginBottom: '12px' }}>
                                    {[
                                        {name: 'Circle', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="#a855f7"><circle cx="12" cy="12" r="10"/></svg>},
                                        {name: 'Square', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="#a855f7"><rect x="3" y="3" width="18" height="18" rx="2"/></svg>},
                                        {name: 'Triangle', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="#a855f7"><path d="M12 3 L22 21 L2 21 Z"/></svg>},
                                        {name: 'Star', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="#a855f7"><path d="M12 2 L15.09 8.26 L22 9.27 L17 14.14 L18.18 21.02 L12 17.77 L5.82 21.02 L7 14.14 L2 9.27 L8.91 8.26 Z"/></svg>},
                                        {name: 'Heart', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="#a855f7"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>},
                                        {name: 'Arrow', icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="#a855f7"><path d="M12 4l-1.41 1.41L16.17 11H4v2h12.17l-5.58 5.59L12 20l8-8z"/></svg>}
                                    ].map(shape => (
                                        <button
                                            key={shape.name}
                                        ...
                                        >
                                            ...
                                            <span style={{ fontSize: '9px', color: '#6b7280', fontWeight: 600 }}>{shape.name}</span>
                                        </button>
                                    ))}
                                </div>
                                {shapes.length > 0 && (
                                    <div>
                                        <div style={{ fontSize: '12px', fontWeight: 600, color: '#6b7280', marginBottom: '8px' }}>Added Shapes ({shapes.length})</div>
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                            {shapes.map(shape => (
                                                <div key={shape.id} style={{
                                                    ...
                                                    <button onClick={() => handleRemoveShape(shape.id)} style={{
                                                        background: '#ef4444',
                                                        color: 'white',
                                                        border: 'none',
                                                        borderRadius: '4px',
                                ...
                                {selectedShapeId && shapes.find(s => s.id === selectedShapeId) && (
                                    <div style={{
                                        ...
                                        <div style={{ fontSize: '12px', fontWeight: 600, color: '#a855f7', marginBottom: '8px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                            <span>✏️</span> Edit Selected Shape
                                        </div>
                                        <div style={{ marginBottom: '12px' }}>
                                            <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Shape Size: {shapes.find(s => s.id === selectedShapeId)?.size}px</div>
                                            <input
                                                type="range"
                                                min="20"
                                                max="100"
                                        ...
                                        <div style={{ marginBottom: '12px' }}>
                                            <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Shape Color</div>
                                            <input
                                                type="color"
                                        ...
                                        <div style={{ marginBottom: '12px' }}>
                                            <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Rotation: {shapes.find(s => s.id === selectedShapeId)?.rotation || 0}°</div>
                                            <input
                                                type="range"
                                                min="0"
                                                max="360"
                                        ...
                                        <div>
                                            <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Shape Style</div>
                                            <div style={{ display: 'flex', gap: '6px' }}>
                                                <button
                                                    ...
                                                    style={{
                                                        flex: 1,
                                                        padding: '8px',
                                                        background: shapes.find(s => s.id === selectedShapeId)?.style === 'filled' ? '#a855f7' : 'white',
                                                        color: shapes.find(s => s.id === selectedShapeId)?.style === 'filled' ? 'white' : '#6b7280',
                                                        border: shapes.find(s => s.id === selectedShapeId)?.style === 'filled' ? 'none' : '1px solid #e5e7eb',
                                                        borderRadius: '6px',
                                                        cursor: 'pointer',
                                                        fontSize: '12px',
                                                        fontWeight: 600
                                                ...
                                                <button
                                                    ...
                                                    style={{
                                                        flex: 1,
                                                        padding: '8px',
                                                        background: shapes.find(s => s.id === selectedShapeId)?.style === 'outline' ? '#a855f7' : 'white',
                                                        color: shapes.find(s => s.id === selectedShapeId)?.style === 'outline' ? 'white' : '#6b7280',
                                                        border: shapes.find(s => s.id === selectedShapeId)?.style === 'outline' ? 'none' : '1px solid #e5e7eb',
                                                        borderRadius: '6px',
                                                        cursor: 'pointer',
                                                        fontSize: '12px',
                                                        fontWeight: 600
                                        ...
                                        <div style={{ fontSize: '10px', color: '#9ca3af', marginTop: '8px', textAlign: 'center' }}>💡 Drag shape on canvas to reposition</div>
                                    </div>
                                )}
                            </div>
                        )}
                        ...
                        {activeTool === 'postout' && (
                            <div>
                                {isSharing ? (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                                        {[
                                            {name: 'Twitter/X', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="#1DA1F2"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>},
                                            ...
                                            {name: 'Instagram', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="url(#instagram-gradient)"><defs><linearGradient id="instagram-gradient" x1="0%" y1="100%" x2="100%" y2="0%"><stop offset="0%" style={{stopColor: '#FD5949', stopOpacity: 1}}/><stop offset="50%" style={{stopColor: '#D6249F', stopOpacity: 1}}/><stop offset="100%" style={{stopColor: '#285AEB', stopOpacity: 1}}/></linearGradient></defs><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>},
                                            {name: 'LinkedIn', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="#0A66C2"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>}
                                        ].map(platform => (
                                            <button
                                                key={platform.name}
                                                onClick={() => handlePostout(platform.name)}
                                                style={{
                                                    padding: '10px',
                                                    background: 'white',
                                                    border: '1px solid #e5e7eb',
                        ...
                        {activeTool === 'brush' && (
                            <div>
                                <div style={{ fontSize: '12px', fontWeight: 600, color: '#6b7280', marginBottom: '8px' }}>Brush Tools</div>
                                <div style={{
                                    background: 'white',
                                    borderRadius: '12px',
                                    padding: '12px',
                                    ...
                                    <div style={{ marginBottom: '12px' }}>
                                        <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Brush Size: {brushSize}px</div>
                                        <input 
                                            type="range" 
                                            min="1" 
                                            max="50" 
                                    ...
                                    <div style={{ marginBottom: '12px' }}>
                                        <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Opacity: {brushOpacity}%</div>
                                        <input 
                                            type="range" 
                                            min="0" 
                                            max="100" 
                                    ...
                                    <div>
                                        <div style={{ fontSize: '11px', color: '#6b7280', marginBottom: '6px' }}>Color</div>
                                        <input 
                                            type="color" 
                                            value={brushColor}
                                ...
                                <button 
                                    ...
                                    style={{
                                        width: '100%',
                                        padding: '10px',
                                        background: isDrawing ? '#ef4444' : '#a855f7',
                                        color: 'white',
                                        border: 'none',
                                        borderRadius: '8px',
                                        cursor: 'pointer',
                                ...
                                >{isDrawing ? '⏹️ Stop Drawing' : '🖌️ Start Drawing'}</button>
                                {isDrawing && (
                                    <div style={{
                                        marginTop: '12px',
                                        padding: '12px',
                                        ...
                                        ✏️ Click and drag on the canvas to draw
                                    </div>
                                )}
                            </div>
                        )}
                                            ...
                                            <button 
                                                onClick={() => {
                                                    ...
                                                    img.onload = () => {
                                                        ...
                                                        const cropContainerWidth = (cropRect.width / 100) * containerRect.width;
                                                        const cropContainerHeight = (cropRect.height / 100) * containerRect.height;
                                                        ...
                                                        const finalCropWidth = Math.min(cropContainerWidth * scaleX, img.naturalWidth - finalCropX);
                                                        const finalCropHeight = Math.min(cropContainerHeight * scaleY, img.naturalHeight - finalCropY);
                ...
                {/* Canvas Area */}
                <main className="edit-canvas-padding" style={{
                    flex: 1,
                    background: '#f3f4f6',
                    display: 'flex',
                    ...
                    <div 
                        ...
                        style={{
                        background: backgroundEnabled ? (selectedGradient || selectedColor || 'linear-gradient(135deg, #FF6B6B 0%, #FFB88C 100%)') : '#f9fafb',
                        borderRadius: '16px',
                        ...
                        position: 'relative',
                        transition: 'all 0.3s ease',
                        cursor: isDrawing ? 'crosshair' : isCropping ? 'crosshair' : (draggingTextId || draggingShapeId) ? 'grabbing' : 'default'
                    }}>
                        {image ? (
                            <>
                                <img
                                    ...
                                    style={{
                                        ...
                                        boxShadow: frameStyle === 'shadow' ? `0 ${shadowIntensity}px ${shadowIntensity * 2}px rgba(0,0,0,0.3)` :
                                                  frameStyle === 'glow' ? `0 0 ${shadowIntensity}px ${shadowIntensity / 2}px rgba(168, 85, 247, 0.6)` :
                                ...
                                {textOverlays.map(overlay => (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            padding: '4px 8px',
                                            borderRadius: '4px',
                                            border: selectedTextId === overlay.id ? '2px dashed rgba(168, 85, 247, 0.8)' : '2px dashed transparent',
                                            background: selectedTextId === overlay.id ? 'rgba(168, 85, 247, 0.15)' : 'transparent',
                                    ...
                                    return (
                                        <div
                                            ...
                                            style={{
                                                ...
                                                padding: '4px',
                                                borderRadius: '4px',
                                                border: selectedShapeId === shape.id ? '2px dashed rgba(168, 85, 247, 0.8)' : '2px dashed transparent',
                                                background: selectedShapeId === shape.id ? 'rgba(168, 85, 247, 0.15)' : 'transparent',
                                                transition: 'border 0.2s, background 0.2s',
                                                textShadow: '2px 2px 4px rgba(0,0,0,0.3)',
                                                lineHeight: 1
                                            }}
                                        >
                                            {shape.style === 'filled' ? shapeEmojis[shape.type] : outlineEmojis[shape.type]}
                                        </div>
                                    );
                                })}
                ...
                <div style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '10px' }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            <div style={{ fontSize: '13px', opacity: 0.9, wordBreak: 'break-all' }}>
                                {sharedLink}
                            </div>
                        </div>
                    </div>
